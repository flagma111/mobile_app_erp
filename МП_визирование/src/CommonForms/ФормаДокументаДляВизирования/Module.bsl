...
Функция УстановитьРешениеНаНаСервере(ВидРешения,UIDЗаявки,Комментарий = "")//Ден  2017 10 03 ---
	Попытка
	...
	Если НЕ Прокси = Неопределено Тогда	
		...
		ИначеЕсли ВидРешения = "Отклонено" Тогда 
			РезультатУстановкиРешения = Прокси.DeclineDocument(UIDЗаявки,Комментарий);
		КонецЕсли;
	КонецЕсли;
...
Процедура Отклонить(Команда)//Ден  2017 10 03 ---
	ПоказатьВводСтроки(новый ОписаниеОповещения("ПослеВводаКомментарияОтклонения",ЭтотОбъект),,"Комментарий");
КонецПроцедуры

&НаКлиенте
Процедура ПослеВводаКомментарияОтклонения(Результат,ДополнительныеПараметры) Экспорт//Ден  2017 10 05 ---
	Если Результат <> Неопределено Тогда
		РезультатУстановкиРешения = УстановитьРешениеНаНаСервере("Отклонено",ДокументUID,Результат);
...
Процедура Согласовать(Команда)//Ден  2017 10 03 ---
	...
	//TODO Картинка успешного согласования!
...
&НаКлиенте
Процедура ПоказатьЦепочкуСоглаования(Команда)//Ден  2017 10 04 ---
	ОткрытьФорму("ОбщаяФорма.ФормаПросмотраЦепочкиСогласования",новый Структура("ДокументUID",ДокументUID),ЭтотОбъект);
КонецПроцедуры
